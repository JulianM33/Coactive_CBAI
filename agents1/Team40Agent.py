...
def writeCsv(agentName, trustDict):
    header = ['member_name', 'trust_score']
    data = []
    for td in trustDict.keys():
        data.append([td, trustDict[td]])
    with open(agentName + '.csv', 'w', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)
...
class Team40Agent(BW4TBrain):
            ...
            if oldMsg is not None and oldMsg != message:
                ...
                if 'Moving to' in oldMsg and 'Opening door of' in message:
                    if removePrefix('Moving to ', oldMsg) == removePrefix('Opening door of ', message):
                        self._updateTrustBy(member, 0.02)
                ...
                if 'Found goal block' in oldMsg and 'Picking up goal block' in message:
                    if parseLocation(oldMsg) == parseLocation(message):
                        if parseBlockVisual(oldMsg) == parseBlockVisual(message):
                            self._updateTrustBy(member, 0.04)
                ...
                if 'Dropped goal block' in oldMsg and 'Moving to' in message:
                    self._updateTrustBy(member, 0.06)
        ...
        for member in self._teamMembers:
            if newMessages[member] is not None:
                ...
                if 'Dropped goal block' in newMessages[member] and self._trustPerMember[member] >= 0.9:
                    ...
                    try:
                        self._activeObjectives.pop(ind)
                    except:
                        pass
                    self._log(member + ' dropped a goal block, updating active list')
    ...
    def decide_on_bw4t_action(self, state: State):
        if self._isFirstAction:
            ...
            memoryTrust = None
            try:
                memoryTrust = readCsv(self._agentName + '.csv')
            except:
                pass
            ...
            for member in self._teamMembers:
                ...
                # Import from csv memory member trusts
                if memoryTrust is not None and member in memoryTrust:
                    self._trustPerMember[member] = memoryTrust[member]
        ...
        # Save updated trusts scores to memory file
        writeCsv(self._agentName, self._trustPerMember)
...
class StrongAgent(BW4TBrain):
        ...
        for member in self._teamMembers:
            if newMessages[member] is not None:
                ...
                if 'Dropped goal' in newMessages[member] and self._trustPerMember[member] >= 0.9:
                    ...
                    if len(self._activeObjectives) == 0:
                        continue
                    ind = indexObjStrEquals(self._activeGoals, parseBlockVisual(newMessages[member]))
                    try:
                        self._activeGoals.pop(ind)
                    except:
                        pass
                    self._log(member + ' dropped a goal block, updating active list')
    ...
    def decide_on_bw4t_action(self, state: State):
        if self._isFirstAction:
            ...
            self._activeObjectives = [goal for goal in state.values()
                                      if 'is_goal_block' in goal and goal['is_goal_block']]
            self._activeGoals = [goal for goal in state.values()
                                      if 'is_goal_block' in goal and goal['is_goal_block']]
        self._isFirstAction = False
        ...
        while True:
                ...
                if len(self._allObjectives) == 0:
                    ...
                    if ind == -1:
                        ...
                        if objEquals(nearby_objects[ind], self._allObjectives[0]):
                            ...
                            self._phase = Phase.DECIDE_ACTION
                self._activeObjectives = self._allObjectives
                self._activeGoals = copy.deepcopy(self._allObjectives)
                self._log('updated active list')
            ...
            if Phase.PLAN_PATH_TO_DROP_OBJECT == self._phase:
                self._navigator.reset_full()
                self._navigator.add_waypoint(self._dropInfo['location'])
                self._phase = Phase.FOLLOW_PATH_TO_DROP_OBJECT
            ...
            if Phase.SEARCH_ROOM == self._phase:
                ...
                if len(state[self.agent_id]['is_carrying']) == 0:
                    indNO, self._indAO = indexObjsEquals(nearby_objects, self._activeObjectives)

                # Carrying a non-priority block, find block of that is currently needed
                if len(state[self.agent_id]['is_carrying']) > 0 \
                        and indexObjEquals(state[self.agent_id]['is_carrying'], self._activeObjectives[0]) == -1:
                    indNO = indexObjEquals(nearby_objects, self._activeObjectives[0])
                ...
                if indNO != -1:
                    self._navigator.reset_full()
                    self._searched_obj = nearby_objects[indNO]
                    self._navigator.add_waypoint(self._searched_obj['location'])
                    self._phase = Phase.FOUND_BLOCK
                    self._sendMessage('Found goal block ' + str(self._searched_obj['visualization'])
                                      + ' at location ' + str(self._searched_obj['location']),
            ...
            if Phase.FOUND_BLOCK == self._phase:
                ...
                if len(self._activeObjectives) == 0:
                    self._phase = Phase.DECIDE_ACTION
                    return None, {}
                # Found object is a priority objective (currently the first to fill)
                self._activeObjectives.pop(self._indAO)
                return GrabObject.__name__, {'object_id': self._searched_obj['obj_id']}
