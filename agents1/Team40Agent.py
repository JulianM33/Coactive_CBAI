        ...
        while True:
            ...
            if Phase.SEARCH_ROOM == self._phase:
                ...
                if nby_obj_ind != -1:
                    ...
                    original_object = self._searched_obj['visualization']

                    # remove colour in the object
                    object_colorblind = {k: v for k, v in original_object.items() if k != 'colour'}

                    self._sendMessage('Found goal block ' + str(object_colorblind)
                                      + ' at location ' + str(self._searched_obj['location']),
                                      agent_name)
                else:
                    action = self._navigator.get_move_action(self._state_tracker)
