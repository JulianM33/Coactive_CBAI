...
class Team40StrongAgent(BW4TBrain):

    def __init__(self, settings:Dict[str,object]):
        super().__init__(settings)
        self._carryingSize = 0
    ...
    def decide_on_bw4t_action(self, state:State):
        ...
        while True:
            ...
            if Phase.DROP_OBJECT == self._phase:
                self._carryingSize -= 1
                self._phase = Phase.DECIDE_ACTION
                self._sendMessage('Dropped goal block ' + str(self._carrying[0]['visualization'])
                                  + ' at location ' + str(self._loc_goal),
                                  agent_name)
                self._activeObjectives.pop(self.object_indices[0])
                return DropObject.__name__, {'object_id': self._carrying[0]['obj_id'], 'max_objects':2}
            ...
            if Phase.SEARCH_ROOM == self._phase:
                ...
                # nby_obj_ind = self._indexObjEquals(nearby_objects, self._activeObjectives)
                nby_obj_ind = self._indexObjectsEquals(nearby_objects, self._activeObjectives)
                obs = self._activeObjectives

                if len(nby_obj_ind) != 0:
                    self.object_indices = nby_obj_ind[0:min(2, len(nby_obj_ind)) - self._carryingSize]
                    self._navigator.reset_full()
                    self._searched_objects = [nearby_objects[i] for i in nby_obj_ind[0:min(2, len(nby_obj_ind))]]

                    for i in range(len(nby_obj_ind)):
                        self._carryingSize += 1
                        self._navigator.add_waypoint(self._searched_objects[i]['location'])
                        self._sendMessage('Found goal block ' + str(self._searched_objects[i]['visualization'])
                                          + ' at location ' + str(self._searched_objects[i]['location']),
                                          agent_name)
            ...
            if Phase.FOUND_BLOCK == self._phase:
                ...
                if action is not None:
                    return action, {}
                self._phase = Phase.EXIT_ROOM
                self._sendMessage('Picking up goal block ' + str(self._searched_objects[0]['visualization'])
                                  + ' at location ' + str(self._searched_objects[0]['location']),
                                  agent_name)
                return GrabObject.__name__, {'object_id': self._searched_objects[0]['obj_id'], 'agent_type': 'strong'}
    ...
    def _indexObjectsEquals(self, nearby_objects, activeObjectives):
        list = []
        for i in range(len(nearby_objects)):
            for j in range(len(activeObjectives)):
                if self._objEquals(nearby_objects[i], activeObjectives[j]):
                    list.append(i)
        return list
        # return -1
